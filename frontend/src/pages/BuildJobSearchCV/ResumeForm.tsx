...
type FormStep = 'summary' | 'education' | 'qualifications' | 'skills' | 
                'experience' | 'certifications' | 'jobPreferences' | 'review';
...
const ResumeForm: React.FC<Props> = ({isFirstTime})  => {
    const [isFirstTimeUser, setIsFirstTimeUser] = useState<boolean>(isFirstTime || true);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isFirstTimeUser===true) {
                ...
                const response = await axiosInstance.put('/user/profile/info', formData);
                console.log('Resume updated:', response.data);
                toast.update(toastId, {
                    render: 'Resume updated successfully!',
                    type: 'success',
    ...
    const validateData = (data: ResumeData) => {
        ...
        const filteredEducation = data.education.filter(
            (edu) => isNonEmptyString(edu.education_level) && isNonEmptyString(edu.study_field) && isNonEmptyString(edu.school_name)
        );
        const filteredExperience = data.experience.filter(
            (exp) => isNonEmptyString(exp.job_title) && isNonEmptyString(exp.company_name)
        );
        const filteredJobPreferences = data.job_preferences.filter(
            (pref) => isNonEmptyString(pref.job_title) && isNonEmptyString(pref.industry)
        );
        const filteredQualifications = data.qualifications.filter(
            (qual) => isNonEmptyString(qual.title)
        );
        const filteredCertifications = data.certifications.filter(
            (cert) => isNonEmptyString(cert.name) && isNonEmptyString(cert.issuing_organization)
        );
        return {
            ...data,
    ...
    return (
        <div>
            <div className="d-flex flex-column min-vh-80">
                ...
                <Container className="py-5 center" style={{ width: '80%', marginTop: '5%' }}>
